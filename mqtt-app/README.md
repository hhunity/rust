...
**マイコン役**（`mqtt-client`）にはキーボード入力の仕組みが無くなりました（以前はデバッグ用に
チャット送信専用の入力スレッドがありましたが、実運用ではプリンタ側に人がコマンドを打つ場面が
無いため削除しています）。パソコン役からの`chat`・`send`・`job`配信を受け取って表示・応答する
だけの、完全に無人稼働のプロセスです。

**パソコン役**（`mqtt-server`）は`reedline-repl-rs`ベースのコマンドREPLです（後述の
「コマンド入力の実装について」参照）。先頭に`/`は付けず、スペース区切りの引数を取ります。
`help`でコマンド一覧、`help <コマンド名>`で個別の使い方が表示されます。

| コマンド | publishするトピック（送信） | 応答として届くトピック（受信） |
|---|---|---|
...
パソコン役のコマンド入力は現在`reedline-repl-rs`クレート（`src/repl_commands.rs`）を
使っています。`if line.starts_with("/job ")`のような自作の分岐チェーンを書く代わりに、
コマンドの引数定義（`clap`ベース）・`help`の自動生成・矢印キーでの履歴/補完を任せられます。

**重要な制約**: `reedline-repl-rs`は内部でカーソル位置をターミナルに問い合わせる処理を行うため、
**本物の対話ターミナルからの起動が前提**です。標準入力をパイプやファイルからリダイレクトする
運用（`echo "job foo" | cargo run --bin mqtt-server`のような自動テスト・スクリプト投入、
出力をログファイルへ丸ごとリダイレクトするだけの運用など）では、コマンド入力スレッドが
`panic`して止まります（MQTTブローカー・配信ロジック自体は生きたまま動き続けるので、
プロセス全体が落ちるわけではありませんが、それ以降コマンドは一切効かなくなります）。
...
`crate::stdin_commands::spawn(...)`に差し替えれば戻ります（`stdin_commands`には`run`
コマンドが無いので、キューに積んだジョブを配信する手段が無くなる点に注意してください。
`stdin_commands`を使うなら、`job_dispatch::run_one`を呼ぶ独自コマンドを追加するか、
以前のように配信専用のバックグラウンドスレッドを別途用意する必要があります）。
コマンドの書き方も元の`/job <内容>`（先頭に`/`、チャットはプレフィックス無し）の形に
戻る点にも注意してください。

**バックグラウンドスレッドからの出力**: `controller`のメインループ（マイコンの接続/切断通知など）は、素の`println!`ではなく
...
| `--queue-file <PATH>` | `-q` | `job_queue.json` |
...
パソコン役の`chat`コマンドで送信します。`--qos 0`〜`--qos 2`でそのメッセージだけQoSを
指定できます（省略時はQoS1）。マイコン役はキーボード入力を持たないので送信側にはなれず、
届いた`<topic>`のメッセージをそのまま表示するだけです。

```
chat こんにちは
chat --qos 0 これはQoS0で送る
```
...
### 印刷ジョブキュー（パソコン役の`job` `run` `queue` `status` `cancel` `retry` `clear`コマンド）

```
job print A4x3    # キューに追加し、その場で配信を試みる（完了待ちで最大10秒ブロックする）
run               # 配信できず止まっているジョブを、先頭から順に進められるだけ全部再開する
run <ID>          # IDを指定して、そのジョブだけを名指しで再開する（先頭でなくてもよい）
queue             # キューの中身と各ジョブの状態を確認
queue failed      # 状態で絞り込んで確認（pending/dispatched/done/failed）
status <ID>       # 1件だけの状態を確認
cancel <ID>       # まだ配信していないジョブを取り消す
retry <ID>        # 失敗したジョブをもう一度Pendingへ戻す
clear             # 完了・失敗済みのジョブをキューから削除して片付ける
```

ジョブは常に「今オンラインになっている全マイコンへの一斉配信」で、**同時に処理するジョブは
常に1件だけ**（並列実行はしない）という設計です。`job`はジョブをキューの末尾に積んだ直後、
**その場で配信を試みます**。宛先が誰もオンラインでない等の理由で配信できなければ、ジョブは
`pending`のまま「止まった」状態になります。この止まったジョブを後から再開させるのが`run`
コマンドです（`src/job_dispatch.rs`の`run_one`・`run_all`関数を、`job`・`run`から呼んでいる）。
`run`はIDを省略すると、Pendingのジョブを先頭から順に**進められるだけ全部**処理します
（あるジョブが「宛先が誰もオンラインでない」という理由で配信できなかった場合、それ以上
進めても同じ結果を繰り返すだけなので、そこで打ち切ります。残りは次に`run`したときに続きから
処理されます）。IDを指定すれば、先頭でなくてもそのジョブ1件だけを名指しで（投入順を
飛び越して）再開できます。Pending以外のジョブ（既にDone/Failed/Dispatched）をIDで
指定した場合は、開始できない旨を返します。

**起動時にファイルから引き継いだ未処理ジョブだけは例外**で、自動では配信されません。
誰かが`job`で新しいジョブを追加するか、`run`を打って初めて処理が始まります（勝手に
印刷が始まると困る、という理由からこうしている。以前はバックグラウンドスレッドが起動時から
自動でキューを監視・消化し続ける設計でしたが、常駐スレッドは廃止しました）。

- **キューの永続化**（`src/job_queue.rs`）: キューの中身（ジョブの並びと状態）は起動時に
  指定したファイル（`--queue-file`、デフォルト`job_queue.json`）へその都度JSONで書き出される。
  プロセスを再起動しても、前回までの未処理ジョブを引き継いでキューの続きに残る
  （SQLite等のDBは使わず、`serde_json`でのファイル書き出しだけで完結させている）。
- **状態遷移**: `pending`（未配信、または配信できず止まっている）→`dispatched`（配信済み、
  完了待ち）→`done`（全員完了）または`failed`（タイムアウトまでに完了報告が揃わなかった）
- **宛先が誰もオンラインでない場合**: `job`・`run`ともジョブを`pending`のままにして、
  配信できなかった旨をその場で返す（「誰かがオンラインになるまで待つ」ことはしない。
  どちらもブロックせず、後でもう一度`run`すればよい設計にしている）
...
**設計判断（「配信前に状態を問い合わせる」の実装方法について）**: 「ジョブを送る前にマイコンの
状態を問い合わせる」という要望に対して、ここでは「その場で1往復リクエスト/レスポンスする」
方式ではなく、**`NSTATUS`をretain＋ワイルドカード購読で常に最新の状態を持ち続け、配信の瞬間に
その名簿を見る**という方式を採っています。MQTTのretainメッセージは購読した瞬間に最新値が
必ず届く仕組みなので、既にずっと購読しているパソコン役にとっては「今すぐ聞きに行く」のと
「今持っている最新値を見る」のは実質同じ新しさになり、往復リクエスト用の新しいメッセージ種別・
タイムアウト処理を増やさずに済みます。もし将来「本当にその場で明示的に聞きに行く」形（例:
`CmdMsg::StatusQuery`を送って返事を待つ）が必要になった場合は、この名簿方式を置き換えるのではなく、
`job_dispatch::idle_targets`が名簿を参照する前に一度問い合わせを挟む、という形で追加できます。
