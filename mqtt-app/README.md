...
1. パソコンが `<topic>/cmd/<宛先名>` トピックへ「このファイルを送りたい」という申し出(`OfferMsg`)をpublish
2. 宛先のマイコンが、自分の待ち受けアドレス（IP＋固定ポート）を `<topic>/data/<自分の名前>`
   トピックへ`AckMsg`として返す（IPアドレスはDHCPで変わることがあるため、返事のたびに毎回調べ直しています）
3. パソコンがそのアドレスへ直接TCP接続し、`id`＋ファイル名＋サイズ＋中身の順に流し込む
4. マイコン側は受信が終わった（または途中で失敗した）ら、結果を `<topic>/data/<自分の名前>`
...
- マイコンは接続時に自分のdataトピック（`<topic>/data/<名前>`）へpresence("online")をpublishし、
  異常切断時にはLast Willで自動的に"offline"が配信される。パソコンはこれをワイルドカード購読して
  「今どのマイコンがオンラインか」を動的な名簿として持っている（固定リストの設定は不要）
- `/job`実行時点でオンラインな全マイコンへ`<topic>/cmd/all`にジョブをpublishし、
  各マイコンからの完了報告（`<topic>/data/<マイコン名>`）が全員分揃うまで入力をブロックして待つ
...
| **Host Application** | 全Edge Node/Deviceをワイルドカード購読して監視し、コマンドを出す「監督者」。自分の生死は専用の1本のトピック（`spBv1.0/STATE/<host_id>`）だけで知らせる | **パソコン**（`<topic>/data/+`で全マイコンを監視し、OFFER/JOBで指示を出す。自分の生死は`<topic>/state/<自分の名前>`で知らせる） |
...
トピックは`<topic>/cmd/<マイコンの名前>`・`<topic>/data/<マイコンの名前>`・
`<topic>/state/<パソコンの名前>`の3種類です（全マイコンへの一斉配信だけは特別に
`<topic>/cmd/all`という名前を使います）。

これは本家Sparkplug Bのトピック（`spBv1.0/<group_id>/<message_type>/<edge_node_id>/
[device_id]`）の考え方をそのまま踏襲したものです。ポイントは2つあります。

1. **種別（`message_type`）が名前より先に来る**：Sparkplug Bでは
   `.../<message_type>/<edge_node_id>`の順で、種別が先・名前が後です。本プロジェクトも
   これに合わせて`<topic>/cmd/<名前>`・`<topic>/data/<名前>`という順にしています
   （`<topic>/<名前>/cmd`のように名前を先にする案もありえますが、本家の順番に揃えました）。
2. **名前が付いたトピックは、常に「その名前の持ち主自身の持ち物」**：Sparkplug Bでは、
   - そこにホストがコマンド（`NCMD`・`DCMD`）を送るのは「そのノードへ命令する」という意味
   - そこにノード自身がデータ（`NBIRTH`・`NDATA`・`NDEATH`・`DBIRTH`・`DDATA`・`DDEATH`）を
     送るのは「自分について報告する」という意味

   というように、**「誰が発行するか（＝メッセージの向き）」はメッセージ種別だけで決まり、
   名前はいつも同じ意味（その持ち主自身）**という設計になっています。

以前の版では、トピック末尾の名前がメッセージの種類によって「宛先」だったり「送信者自身」
だったりして分かりにくい、という指摘を受けました。そこで今回、Sparkplug Bのこの発想に
倣って作り直し、名前の意味を常に1つ（その持ち主自身のチャンネル）に統一しています。

- `<topic>/cmd/<名前>` … ホスト（パソコン）が、名前のマイコンへ向けて送るコマンド。
  `OfferMsg`（ファイル送信の申し出）が乗る
- `<topic>/cmd/all` … ホストが、全マイコンへ一斉配信するコマンド。`JobMsg`が乗る
  （`"all"`は「全員」を表す特別な名前で、実在するマイコン名ではない）
- `<topic>/data/<名前>` … 名前のマイコン自身が、自分について報告するデータ。
  `PresenceMsg`・`AckMsg`・`ReceivedMsg`・`DoneMsg`の4種類全部がここに乗る
- `<topic>/state/<名前>` … 名前のパソコン自身が、自分の生死を報告するデータ
  （Sparkplug Bの`STATE`に相当）。中身は`PresenceMsg`そのまま
...
`std::variant`やタグ付きunionに近いものです。`state`トピックだけは乗るメッセージが
`PresenceMsg`の1種類しか無いので、こうした`enum`で包んでいません）。

| トピック | 乗るメッセージ（`"type"`） | 送信元→送信先 | ペイロード（JSON） |
|---|---|---|---|
| `<topic>/cmd/<名前>` | `"file_offer"` | パソコン → 名指しした1台のマイコン | `{"type":"file_offer","id","from","filename","size","seq"}` |
| `<topic>/cmd/all` | `"job"` | パソコン → 全マイコンへの一斉配信 | `{"type":"job","id","from","content","seq"}` |
| `<topic>/data/<名前>` | `"presence"` | マイコン → 見ている全員 | `{"type":"presence","status","seq"}`（`status`は`"online"`か`"offline"`） |
| `<topic>/data/<名前>` | `"file_ack"` | マイコン → 見ている全員 | `{"type":"file_ack","id","host","port","seq"}` |
| `<topic>/data/<名前>` | `"file_received"` | マイコン → 見ている全員 | `{"type":"file_received","id","status","size","seq"}`（`status`は`"ok"`か`"failed"`） |
| `<topic>/data/<名前>` | `"job_done"` | マイコン → 見ている全員 | `{"type":"job_done","id","seq"}` |
...
マイコン側は自分の名前が付いた`cmd`トピック（＋`cmd/all`）だけを購読すればよく、他のマイコン
宛てのOFFERを気にする必要がありません。パソコン役は`<topic>/data/+`とワイルドカード購読すれば、
全マイコンの報告をまとめて拾えます。マイコン役も同様に`<topic>/state/+`をワイルドカード購読して、
パソコンの生死を知ることができます。
...
**注意（`state`トピックの限界）**: マイコン側の`presence`（LWT）は、ブローカーとマイコンが
別プロセスなので正しく機能します（マイコンが死んでもブローカーは生きていて、代わりに
"offline"をpublishしてくれる）。一方パソコン側は、**ブローカーとコントローラ（パソコン役）が
...
（中身の種類が違っても）必ず全部見えるはずだからです。例えば`<topic>/data/<名前>`には
...
逆に、別のトピックへ流れるメッセージ（例えば他のマイコン宛ての`<topic>/cmd/他の名前`）は
見えないので、それを同じカウンタに混ぜてしまうと、番号が「飛んで」見えてしまい、実際は
何も欠落していないのに誤って警告が出てしまいます（もともとの2プロジェクト構成での実装中に、
実際にこの誤検知が起きたことがあります）。

具体的には4系統のトピックに対応する4系統のカウンタ/トラッカーを用意しています。
`<topic>/cmd/<名前>`（OFFER。宛先ごとに別トピック）、`<topic>/cmd/all`（JOB。全マイコン
共通の1トピック）、`<topic>/data/<名前>`（presence・ACK・RECEIVED・DONEをまとめた1トピック）、
`<topic>/state/<名前>`（パソコンの生死。パソコンごとに別トピック）の4つです。パソコン役は
「OFFER・JOBを送る側」「各マイコンのdataトピックを受け取る側」「自分のstateを送る側」、
