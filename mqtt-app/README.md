...
| `run [ジョブID]` | IDを指定すればそのジョブ、省略すれば先頭で止まっている(Pendingの)ジョブを再開 | 宛先マイコンの完了報告を待って結果を表示（最大10秒） |
...
run               # 配信できず止まっている先頭のジョブがあれば、1件だけ再開する
run <ID>          # IDを指定して、そのジョブを名指しで再開する（先頭でなくてもよい）
...
コマンドです（`src/job_dispatch.rs`の`run_one`関数を、`job`・`run`の両方から呼んでいる）。
`run`はIDを省略するとキューの先頭にあるPendingジョブを処理しますが、IDを指定すれば
先頭でなくても、そのジョブを名指しで（投入順を飛び越して）再開できます。Pending以外の
ジョブ（既にDone/Failed/Dispatched）をIDで指定した場合は、開始できない旨を返します。
