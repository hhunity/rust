...
| `job <内容...>` | ジョブキューに積むだけ（即座に返る。配信は下記「印刷ジョブキュー」参照） | なし（結果は`queue`・`status`で確認） |
...
### 印刷ジョブキュー（パソコン役の`job` `queue` `status` `cancel` `retry` `clear`コマンド）

```
job print A4x3    # キューに追加（即座に返る）
...
- **宛先が誰もオンラインでない場合**: そのジョブは`pending`のまま、誰かがオンラインになるまで
  キュー全体をブロックして待つ（後続のジョブも先には進まない）
- マイコンは接続時に自分の`NBIRTH`トピック（`<topic>/NBIRTH/<名前>`）へ接続通知をpublishし、
  ...
  （固定リストの設定は不要）。配信するマイコンの一覧は、実際に配信する瞬間のこの名簿から決まる
- 配信後、一定時間（デフォルト10秒）応答が無いマイコンがいた場合は`failed`として次のジョブに進む
  （自動リトライはしない）
...
└── src/
    ...
    ├── job_worker.rs       … job_queueを順番に処理して配信するバックグラウンドスレッド（mqtt-serverだけが使う）
    └── bin/
