...
| コマンド | 使える側 | 内容 | 実際に行われる処理 |
|---|---|---|---|
| `<テキスト>`（プレフィックス無し） | パソコン・マイコン共通 | 普通のチャット発言 | `<名前>: <テキスト>`という文字列を、QoS1（AtLeastOnce）で`<topic>`へpublishするだけ |
| `/qos0 <テキスト>` | パソコンのみ | QoS0でチャット発言 | 同上だが、QoS::AtMostOnce（送りっぱなし）でpublish |
| `/qos1 <テキスト>` | パソコンのみ | QoS1でチャット発言（プレフィックス無しと同じ） | 同上、QoS::AtLeastOnceでpublish |
| `/qos2 <テキスト>` | パソコンのみ | QoS2でチャット発言 | 同上、QoS::ExactlyOnce（正確に1回）でpublish |
| `/send <宛先の名前> <ファイルパス>` | パソコンのみ | 指定したマイコン1台へファイル送信を申し出る | ①ファイルの存在確認 → ②送信予定を`pending_offers`に記録 → ③`OfferMsg`（`NCMD`）を`<topic>/NCMD/<宛先>`へpublish → ④相手からのACK（`NDATA`）が届いたら、生TCPで実際にファイルの中身を送信 |
| `/job <内容>` | パソコンのみ | オンライン中の全マイコンへジョブを一斉配信 | ①その瞬間の名簿（オンラインなマイコン）を確定 → ②`JobMsg`（`NCMD`）を`<topic>/NCMD/all`へpublish → ③全員分の完了報告（`NDATA`の`job_done`）が揃うか、タイムアウト（デフォルト10秒）になるまで、この入力用スレッドをブロックして待つ |
