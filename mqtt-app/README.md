...
**マイコン役**（`mqtt-client`）の入力用スレッドは、今まで通り「行を入力して`Enter`」の
チャット専用（プレフィックス無し）です。`Ctrl+D`で終了できます。

| コマンド | publishするトピック（送信） | 応答として届くトピック（受信） |
|---|---|---|
| `<テキスト>` | `<topic>`（QoS1、`{"type":...}`ではなく`<名前>: <テキスト>`という生の文字列） | なし |
...
| `job <内容...>` | ジョブキューに積むだけ（即座に返る。自動では配信されない。下記「印刷ジョブキュー」参照） | なし（結果は`queue`・`status`で確認） |
| `run` | キューの先頭にあるPendingジョブが1件あれば配信 | 宛先マイコンの完了報告を待って結果を表示（最大10秒） |
...
マイコン役は行を入力してEnterを押すと`<名前>: <入力内容>`が`<topic>`にpublishされます
（デバッグ用の簡易機能。`Ctrl+D`で送信スレッドを終了できます）。パソコン役は`chat`コマンドを
使い、`--qos 0`〜`--qos 2`でそのメッセージだけQoSを指定できます（省略時はQoS1）。
...
```
job print A4x3    # キューに追加（即座に返る。まだ配信はされない）
run               # キューの先頭にあるPendingジョブを1件だけ配信し、完了を待つ
...
常に1件だけ**（並列実行はしない）という設計です。`job`はジョブをキューの末尾に積むだけで、
**自動では一切配信されません**（起動時にファイルから引き継いだ未処理ジョブも同様）。
配信は`run`コマンドを打つたびに、その時点でのキューの先頭にあるPendingジョブを1件だけ
処理する形で行われます（`src/job_dispatch.rs`の`run_one`関数。以前はバックグラウンドスレッドが
自動でキューを監視・消化し続ける設計でしたが、「勝手に配信してほしくない」という理由から
常駐スレッドは廃止し、`run`を明示的に呼んだときだけ処理する形に変更しました）。
...
- **状態遷移**: `pending`（未配信）→`dispatched`（配信済み、完了待ち）→`done`（全員完了）
  または`failed`（タイムアウトまでに完了報告が揃わなかった）
- **宛先が誰もオンラインでない場合**: `run`はジョブを`pending`のままにして、配信できなかった
  旨をその場で返す（以前のように「誰かがオンラインになるまで待つ」ことはしない。`run`は
  ブロックせず、後でもう一度`run`すればよい設計にしている）
- マイコンは接続時に自分の`NBIRTH`トピック（`<topic>/NBIRTH/<名前>`）へ接続通知をpublishし、
  ...
  （固定リストの設定は不要）。配信するマイコンの一覧は、実際に`run`した瞬間のこの名簿から決まる
- `run`は配信後、一定時間（デフォルト10秒）応答が無いマイコンがいた場合は`failed`にして
  結果を返す（自動リトライはしない。やり直したければ`retry`してから`run`する）
