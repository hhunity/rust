...
| `/job <内容>` | パソコンのみ | ジョブキューに積むだけ（即座に返る。配信は下記「印刷ジョブキュー」参照） | なし（結果は`/queue`・`/status`で確認） |
| `/queue [状態]` | パソコンのみ | なし | キュー内のジョブ一覧を表示（`pending`/`dispatched`/`done`/`failed`を付けるとその状態だけに絞れる） |
| `/status <ジョブID>` | パソコンのみ | なし | 指定した1件のジョブの状態を表示 |
| `/cancel <ジョブID>` | パソコンのみ | なし | 未配信（Pending）のジョブをキューから取り消す |
| `/retry <ジョブID>` | パソコンのみ | なし | 失敗（Failed）したジョブをPendingへ戻し、再配信させる |
| `/clear` | パソコンのみ | なし | 完了（Done）・失敗（Failed）のジョブをキューから削除して履歴を片付ける |
...
### 印刷ジョブキュー（パソコン役の`/job` `/queue` `/cancel`コマンド）
