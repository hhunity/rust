//! # mqtt-client（マイコン役）
//!
//! MQTTブローカーへ接続し、ファイル受信とジョブ実行を行うマイコン役専用のプログラム。
//! パソコン役とやり取りするメッセージの一覧（API仕様にあたるもの）は [`messages`] モジュールを
//! 参照。seq番号による欠落検知の仕組みは [`seq`] を参照。
//!
//! - [`file_transfer`] — 生TCPでのファイル受信ロジックと、自分のIP自動検出
